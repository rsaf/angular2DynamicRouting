...
io.on('connection', function (socket) {
    ...
    socket.on('join', function (user) {
        ...
        redis.subscribe(['chat.message', 'chat.explosion','chat.balloon','chat.clean'], function (err, count) {
...
app.get('/', function(request, response){
  ...
  response.end('200',{status:'hit backend----'});
});