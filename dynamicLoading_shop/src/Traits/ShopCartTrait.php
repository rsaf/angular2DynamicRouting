...
trait ShopCartTrait
{
    ...
    {
        return $this->hasMany(Config::get('shop.item'), 'cart_id','cart_id');
    }
      ...
      // dd('a');
        ...
        if (empty($cartItem)) {
            ...
            $cartItem = call_user_func( Config::get('shop.item') . '::create', [
                ...
                'cart_id'       => $this->attributes[$this->primaryKey],
    ...
    public function placeOrder($statusCode = null)
    {
        ...
        for ($i = count($this->items) - 1; $i >= 0; --$i) {
            // Attach to order
            $this->items[$i]->order_id  = $order->order_id;
            // Remove from cart
    ...
    public function clear()
    {
        DB::table(Config::get('shop.item_table'))
            ->where('cart_id', $this->attributes[$this->primaryKey])
            ->delete();
        $this->resetCalculations();
        return $this;
    }
