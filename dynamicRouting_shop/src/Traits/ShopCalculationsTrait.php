...
trait ShopCalculationsTrait
{
    ...
    {
        return 'shop_' . $this->table . '_' . $this->attributes[$this->primaryKey] . '_calculations';
    }
    ...
    private function runCalculations()
    {
        ...
        // dd(Config::get('shop.item_table'));
        // dd($this->attributes[$this->primaryKey]);
        $this->shopCalculations = DB::table($this->table)
            ...
            ->join(
                ...
                Config::get('shop.item_table') . '.' . ($this->table == Config::get('shop.order_table') ? 'order_id' : $this->primaryKey),
                '=',
                $this->table . '.' . $this->primaryKey
            )
            ->where($this->table . '.' . $this->primaryKey, $this->attributes[$this->primaryKey])
            ->first();
        if (Config::get('shop.cache_calculations')) {
            Cache::put(
                $cacheKey,
